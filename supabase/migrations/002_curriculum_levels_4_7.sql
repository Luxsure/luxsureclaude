-- Migration: add curriculum metadata (XP, lesson type, module badges, level number)
-- and seed Levels 4-7 (Outils & Écosystème, Création Visuelle, Agents & Automatisation,
-- Maîtrise Professionnelle) of the AI Fluency Academy curriculum.
--
-- Run this in the Supabase SQL Editor AFTER schema.sql + seed.sql have already been applied.
-- Safe to run once on the existing production database (odgcybmrxzdscmkctwyn).

-- ============================================================
-- SCHEMA CHANGES
-- ============================================================
alter table public.courses add column if not exists level_number integer;

alter table public.modules add column if not exists icon text;
alter table public.modules add column if not exists badge_title text;
alter table public.modules add column if not exists badge_icon text;

alter table public.lessons add column if not exists xp integer not null default 10;
...
begin
  if not exists (
    ...
    alter table public.lessons
      ...
      check (lesson_type in ('theory', 'practice', 'interactive', 'project', 'battle'));
  end if;
end $$;

-- ============================================================
-- SEED DATA — appended below by the integration step
-- ============================================================
-- ============================================================
-- COURSES — Levels 4-7
-- ============================================================
